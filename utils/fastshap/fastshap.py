...
def evaluate_explainer(explainer, normalization, x, grand, null, num_players,
    ...
    pred: (batch_size, num_classes, height/2, width/2) UNet configuration
    """
    # ipdb.set_trace()
    pred = explainer(x)
    ...
    if len(pred.shape) == 4:
        ...
        pred = pred.reshape(len(x), -1, num_players)  # (batch_size, num_classes, num_players)
        pred = pred.permute(0, 2, 1)  # (batch_size, num_players, num_classes)
    else:
        # Tabular.
        image_shape = None
    ...
    total = pred.sum(dim=1)  # (batch_size, num_classes)
...
def generate_validation_data(val_set, imputer, validation_samples, sampler,
                             batch_size, link, device, num_workers, pin_memory):
    '''
...
class FastSHAP:
    ...
    def train(self,
        ...
        explainer.train()
        device_imputer = next(imputer.surrogate.parameters()).device
        device_explainer = next(explainer.parameters()).device
        ...
        grand_train = calculate_grand_coalition(
            train_set, imputer, batch_size * num_samples, link, device_imputer,
            num_workers, pin_memory).cpu()
        grand_val = calculate_grand_coalition(
            val_set, imputer, batch_size * num_samples, link, device_imputer,
            num_workers, pin_memory).cpu()

        # Null coalition.
        with torch.no_grad():
            zeros = torch.zeros(1, num_players, dtype=torch.float32,
                                device=device_imputer)
            null = link(imputer(train_set[0][0].unsqueeze(0).to(device_imputer), zeros)).to(device_explainer)
            if len(null.shape) == 1:
        ...
        val_S, val_values = generate_validation_data(val_set, imputer, validation_samples, sampler,
                                                     batch_size * num_samples, link, device_imputer, num_workers,
                                                     pin_memory)
        ...
        for epoch in range(max_epochs):
            ...
            for x, grand in batch_iter:
                ...
                # Move to device.
                x = x.to(device_imputer)
                S_imputer = S.to(device_imputer)
                S_explainer = S.to(device_explainer)
                grand = grand.to(device_explainer)
                # ipdb.set_trace()
                ...
                with torch.no_grad():
                    values = link(imputer(x_tiled, S_imputer)).to(device_explainer)

                # ipdb.set_trace()
                # Evaluate explainer.
                pred, total = evaluate_explainer(
                    explainer, normalization, x.to(device_explainer), grand, null, num_players)

                # Calculate loss.
                S_explainer = S_explainer.reshape(batch_size, num_samples, num_players)
                values = values.reshape(batch_size, num_samples, -1)

                """
                null: (1, num_classes)
                pred: (batch_size, num_players, num_classes)
                S: (batch_size, num_samples, num_players)
                
                values: (batch_size, num_samples, num_classes)



                null: torch.Size([1, 10])
                pred: torch.Size([4, 196, 640])
                S: torch.Size([4, 2, 196])
                
                values: torch.Size([4, 2, 10])
                """
                # (1, num_classes) + (batch, num_samples, num_players) @ (batch, num_players, num_classes)

                approx = null + torch.matmul(S_explainer, pred)
                loss = loss_fn(approx, values)
                if eff_lambda:
        ...
        with torch.no_grad():
            ...
            if self.normalization:
                grand = calculate_grand_coalition(
                    x, self.imputer, len(x), self.link, device, 0, True)
            else:
                grand = None

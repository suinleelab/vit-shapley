...
class KLDivLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        Args:
          pred: in log scale
          target: not in log scale. usually output by softmax
        '''
...
class DatasetKeyOnly(Dataset):
    ...
    def __init__(self, dataset, key_list=None):
        assert isinstance(dataset, Dataset)
        self.dataset = dataset
        self.key_list = key_list

    def __getitem__(self, index):
        return [self.dataset[index][key] for key in self.key_list]

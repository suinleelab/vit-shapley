...
def validate(surrogate, loss_fn, data_loader):
    ...
    with torch.no_grad():
        ...
        for x, y, S in data_loader:
            ...
            # import ipdb
            # ipdb.set_trace()
            # x = surrogate.surrogate_resizer(x).to(device)
            x = x.to(device)
            y = y.to(device)
            S = S.to(device)
            pred = surrogate(x, S)
...
class ImageSurrogate(ImageImputer):
    ...
    def __init__(self, surrogate, width, height, superpixel_size):
        ...
        # self.surrogate_resizer = torchvision.transforms.Resize(size=(self.height, self.width))
    ...
    def train_original_model(self,
        ...
        device_surr = next(surrogate.parameters()).device
        device_original = next(original_model.parameters()).device
        optimizer = optim.Adam(surrogate.parameters(), lr=lr)
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer, factor=lr_factor, patience=lookback // 2, min_lr=min_lr,
            verbose=verbose)
        ...
        for epoch in range(max_epochs):
            ...
            for (x,) in batch_iter:
                # Prepare data.
                x_original = x.to(device_original)
                # x_surr = self.surrogate_resizer(x)
                x_surr = x.to(device_surr)

                # Get original model prediction.
                with torch.no_grad():
                    y = original_model(x_original).to(device_surr)

                # Generate subsets.
                S = sampler.sample(batch_size).to(device=device_surr)

                # print(x_surr.shape, S.shape)
                # Make predictions.
                pred = self.__call__(x_surr, S)
                # ipdb.set_trace()
                loss = loss_fn(pred, y)
                #

...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, mask_location='pre-softmax', output_attentions=False):
        B, N, C = x.shape
        ...
        if mask is not None:
            ...
            if mask_location == 'pre-softmax':
                ...
                attn_permute = attn_permute + (1 - mask).masked_fill(mask != 1, float("-Inf")).unsqueeze(0).unsqueeze(3)
                ...
                attn = attn.clone()
                attn_permute = attn.permute((1, 0, 3, 2))
                # attn_permute[:, mask != 1] = torch.tensor(float(0)).type(attn_permute.dtype).to(attn.device)
                attn_permute = attn_permute * mask.unsqueeze(0).unsqueeze(3)
                attn = attn_permute.permute((1, 0, 3, 2))
            else:
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, mask_location='pre-softmax', output_attentions=False):
        attn_output = self.attn(self.norm1(x), mask, mask_location, output_attentions=output_attentions)  # [0]
        x = x + self.drop_path(attn_output[0])
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        output = (x,) + attn_output[1:]
        return output
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, mask=None, mask_location='pre-softmax', residual=[], output_attentions=False,
        ...
        # Attention block propagation.
        all_hidden_states = (x,) if output_hidden_states or len(residual) != 0 else None
        all_self_attentions = () if output_attentions else None
        for i, layer_module in enumerate(self.blocks):
            layer_outputs = layer_module(x=x, mask=mask, mask_location=mask_location,
                                         output_attentions=output_attentions)
            x = layer_outputs[0]
            if output_attentions:
                all_self_attentions = all_self_attentions + (layer_outputs[1],)
            if output_hidden_states or len(residual) != 0:
                # ipdb.set_trace()
                all_hidden_states = all_hidden_states + (x,)

        for i in residual:
            x = x + all_hidden_states[i]
        if output_hidden_states is None:
            all_hidden_states = None

        # x, _ = self.blocks(x, mask, mask_location)
        x = self.norm(x)

        if self.dist_token is None:
            x, x_others = self.pre_logits(x[:, 0]), self.pre_logits(x[:, 1:])
        else:
            x, x_others = (x[:, 0], x[:, 1]), x[:, 2:]
    ...
    def forward(self, x, mask=None, mask_location='pre-softmax', residual=[], output_attentions=False,
                output_hidden_states=False):
        x = self.patch_embed(x)
        blocks_forwarded = self.forward_features(x, mask, mask_location, residual=residual,
                                                 output_attentions=output_attentions,
        ...
        return {'x': x,
                ...
                'hidden_states': blocks_forwarded['hidden_states'],
                'self_attentions': blocks_forwarded['self_attentions']}
